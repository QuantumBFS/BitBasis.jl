...
@show allone(0b1011, mask) # true if all masked positions are 1
@show anyone(0b1011, mask) # true if any masked positions is 1
@show ismatch(0b1011, mask, 0b1001)  # true if mask outed position matches `0b1001`
...
@show btruncate(0b1011, 2);  # only the first (right most) two qubits are retained
